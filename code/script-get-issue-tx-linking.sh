...
parser=$current_folder'/parser-dvdrippler-issue-details.pl'

all_linking_files=($current_folder'/linked-accounts-btc.txt' $current_folder'/linked-accounts-ltc.txt' $current_folder'/linked-accounts-trc.txt' $current_folder'/linked-accounts-nmc.txt')
			...
			then
				...
				do
					...
					then
						...
						echo -e "rfYv1TXnwgDDK4WQNbFALykYuEBnrR4pDX\t$btc_addr_gw" >> $linking_file
					else
						#echo -e "$xrp_addr_user\t$addr"
						echo -e "$xrp_addr_user\t$addr"	 >> $linking_file	
					fi
